...
func (c *SpotifyCommand) Execute(ctx context.Context) error {
	if c.DBPath == "" {
		cfg, err := config.Load()
		if err != nil {
	...
	_, err = service.ImportSpotify(ctx, c.SourceDir, options)
	if err != nil {
...
func HandleSpotifyImport(ctx context.Context, dbPath, sourceDir string, dryRun, force bool) error {
	cmd := SpotifyCommand{
		DBPath:    dbPath,
		SourceDir: sourceDir,
		DryRun:    dryRun,
